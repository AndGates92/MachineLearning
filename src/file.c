...
FILE * file_open (const char * filename, const char * permission) {
	...
	log_info("File %s in function %s at line %0d: Attempting to open file %s for %s\n", __FILE__, __func__, __LINE__,  filename, mode);
	...
	log_info("File %s in function %s at line %0d: Opened file %s for %s. File descriptor %0d\n", __FILE__, __func__, __LINE__,  filename, mode, fileno(fid));
...
void file_close (FILE * fid) {
	if (fid!=NULL) {
		log_info("File %s in function %s at line %0d: Closing file associated with file descriptor %0d\n", __FILE__, __func__, __LINE__,  fileno(fid));
		fclose(fid);
	}
}
